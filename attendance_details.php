...
if (!$recordId || $recordId < 1) redirectTo('admin_panel.php');

$stmt = $conn->prepare(
    "SELECT ae.id, ae.user_id, ae.action_type, ae.latitude, ae.longitude,
...
<style>#detailMap{height:420px;width:100%;border-radius:12px;background:#e2e8f0}</style>
</head>
<body>
<header class="topbar">
<div><h1>FieldTrack</h1><p>Attendance Record #<?= (int)$recordId ?></p></div>
...
<section class="details-card">
<h2>Recorded GPS Location</h2>
<p>This is the location captured automatically when attendance was marked.</p>
...
const action = <?= json_encode((string)$record['action_type']) ?>;
const map = L.map('detailMap').setView([lat,lng],16);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{
    maxZoom:19,
    attribution:'&copy; OpenStreetMap contributors'
}).addTo(map);
L.marker([lat,lng]).addTo(map).bindPopup(action + ' attendance location').openPopup();
</script>
</body>
</html>
