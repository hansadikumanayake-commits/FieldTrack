...
$countStmt = $conn->prepare(
    "SELECT
        SUM(CASE WHEN status IN ('submitted','resubmitted') THEN 1 ELSE 0 END) AS pending_count,
...
$stmt = $conn->prepare(
    "SELECT
     ...
     GROUP BY
        ...
        fo.username
     ORDER BY
        CASE WHEN ws.status IN ('submitted','resubmitted') THEN 0 ELSE 1 END,
        ws.week_start DESC,
        ws.id DESC"
);
...
<header class="topbar">
    <div>
        <h1>FieldTrack</h1>
        <p>Admin Officer Dashboard — <?= h(currentDisplayName()) ?></p>
    </div>
        ...
        <?php foreach ($rows as $row): ?>
            ...
            $statusClass = in_array($status, ['submitted','resubmitted'], true)
                ...
                : (str_contains($status, 'rejected') || $status === 'returned_for_correction'
                    ? 'status-rejected'
                    : (in_array($status, ['pending_manager_review','final_approved'], true)
                        ? 'status-approved'
                        : ''));
            ?>
